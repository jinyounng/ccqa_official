...
def apply_new_self_consistency(results):
    ...
    for item in tqdm(results, desc="새로운 self-consistency 적용 중"):
        ...
        if not most_common_answers:
            ...
            # 최다 득표 답변의 개수가 2개 이하면 NLI 점수로 결정
            if best_count <= 1:
                # NLI 점수로 결정하기
                all_nli_scores = []
                ...
                for idx, ans in valid_answers:
                    if "similarity_scores" in item and f"question_{idx}" in item["similarity_scores"]:
                        # NLI 점수(entailment_min) 가져오기
                        nli_score = item["similarity_scores"][f"question_{idx}"].get("precision", 0)
...
def main():
    ...
    """
    # 입력 폴더 경로
    INPUT_FOLDER = "/data3/jykim/Projects/CCQA_official/Benchmarks/Multi-Arith/Result/precision_nli_separate"
    
    # 출력 폴더 경로
    OUTPUT_DIR = "/data3/jykim/Projects/CCQA_official/Benchmarks/Multi-Arith/Result/precision_vote_results"
