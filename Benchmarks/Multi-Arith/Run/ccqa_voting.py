...
def apply_original_self_consistency(results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for item in tqdm(results, desc="Applying original self-consistency", leave=False):
...
def apply_precision_nli_self_consistency(results: List[Dict[str, Any]], weights: List[int]) -> List[Dict[str, Any]]:
    """
    Precision 기반 투표에서 동률 시 NLI 점수로 해결하는 self-consistency 적용
    ...
    Returns:
        precision 기반, NLI 동률 해결 self-consistency가 적용된 결과 데이터 리스트
    """
    updated_results = []
    ...
    for item in tqdm(results, desc=f"Applying precision-nli SC [{weight_info}]", leave=False):
        updated_item = item.copy()
        
        # most_similar_idxs가 있는지 확인
        if "most_similar_idxs" not in item or not item["most_similar_idxs"]:
            # most_similar_idxs가 없으면 기존 처리 방식 사용
            updated_item[f"precision_nli_sc_method_{weight_info}"] = "fallback_to_first"
            if "response_1" in item:
                answer = extract_numerical_answer(item["response_1"])
                updated_item[f"precision_nli_sc_answer_{weight_info}"] = answer
                updated_item[f"precision_nli_sc_source_{weight_info}"] = "first_response"
        ...
        answer_votes = []
        extracted_answers = []
        extracted_answers_with_idx = []  # 인덱스와 함께 정답 저장
        ...
        for idx, response_idx in enumerate(top_indices[:len(weights)]):
            ...
            if response_key in item and item[response_key]:
                ...
                extracted_answers.append(answer)
                extracted_answers_with_idx.append((response_idx, answer))
                if answer:
                    # 해당 인덱스의 가중치만큼 표 부여
                    weight = weights[idx]
        ...
        # 정답 투표 결과 저장
        updated_item[f"precision_nli_vote_indices_{weight_info}"] = top_indices
        updated_item[f"precision_nli_vote_answers_{weight_info}"] = answer_votes
        updated_item[f"precision_nli_vote_weights_{weight_info}"] = weights  # 사용된 가중치 체계 저장
        updated_item[f"precision_nli_vote_extracted_{weight_info}"] = extracted_answers  # 추출된 원본 답변 저장
        ...
        if answer_votes:
            ...
            if len(tied_answers) > 1:
                # 동률인 경우 Precision 점수로 결정
                tied_answer_indices = []
                
                # 동률이 있는 답안들의 인덱스 찾기
                for tied_answer in tied_answers:
                    # 해당 답변을 제공한 첫 번째 인덱스 찾기
                    for idx, ans in extracted_answers_with_idx:
                        if ans == tied_answer:
                            tied_answer_indices.append(idx)
                            break
                
                
                # 동률인 답변들에 대해 entailment_max 점수 비교
                best_entailment_score = -1
                best_answer = None
                best_idx = None

                for idx in tied_answer_indices:
                    if "similarity_scores" in item and f"question_{idx}" in item["similarity_scores"]:
                        entailment_score = item["similarity_scores"][f"question_{idx}"].get("entailment_max", 0)
                        if entailment_score > best_entailment_score:
                            best_entailment_score = entailment_score
                            best_answer = next((ans for i, ans in extracted_answers_with_idx if i == idx), None)
                            best_idx = idx
                
                if best_answer:
                    # entailment_max 점수로 결정된 답변 저장
                    updated_item[f"precision_nli_sc_answer_{weight_info}"] = best_answer
                    updated_item[f"precision_nli_sc_source_{weight_info}"] = f"entailment_tiebreaker_score_{best_entailment_score:.4f}_idx_{best_idx}"
                    updated_item[f"precision_nli_sc_method_{weight_info}"] = "entailment_vote_entailment_tiebreaker"
                else:
                    # Precision 점수가 없는 경우, 첫 번째 동률 답변 선택
                    updated_item[f"precision_nli_sc_answer_{weight_info}"] = tied_answers[0]
                    updated_item[f"precision_nli_sc_source_{weight_info}"] = "first_of_tied_no_precision"
                    updated_item[f"precision_nli_sc_method_{weight_info}"] = "precision_vote_first_tiebreaker"
            else:
                # 동률이 없으면 최다 득표 정답 선택
                updated_item[f"precision_nli_sc_answer_{weight_info}"] = top_answer
                updated_item[f"precision_nli_sc_source_{weight_info}"] = f"votes_{top_count}_of_{sum(vote_counts.values())}"
                updated_item[f"precision_nli_sc_method_{weight_info}"] = "precision_vote_no_tie"
        else:
...
def process_file(file_path: str, weight_schemes: List[List[int]]) -> Dict:
    """
    파일을 처리하여 CoT, Self-Consistency, Precision-NLI Self-Consistency를 적용합니다.
    ...
    try:
        ...
        # 2. precision-nli self-consistency 적용
        precision_nli_results = sc_results
        ...
        for weights in weight_schemes:
            ...
            # precision-nli 기반 self-consistency 적용
            precision_nli_results = apply_precision_nli_self_consistency(precision_nli_results, weights)
            
            # precision-nli 정확도 계산
            pnli_correct_count = 0
            pnli_valid_count = 0
            
            for item in precision_nli_results:
                pnli_answer = item.get(f"precision_nli_sc_answer_{weight_info}")
                correct_answer = item.get("correct_answer")
                
                if pnli_answer is not None and correct_answer is not None:
                    pnli_valid_count += 1
                    
                    # 숫자만 추출하여 비교
                    pnli_numeric = re.sub(r'[^\d.]', '', str(pnli_answer).strip())
                    correct_numeric = re.sub(r'[^\d.]', '', str(correct_answer).strip())
                    
                    try:
                        if pnli_numeric and correct_numeric:
                            pnli_float = float(pnli_numeric)
                            correct_float = float(correct_numeric)
                            if abs(pnli_float - correct_float) < 1e-5:
                                pnli_correct_count += 1
                                item[f"precision_nli_sc_is_correct_{weight_info}"] = True
                            else:
                                item[f"precision_nli_sc_is_correct_{weight_info}"] = False
                        else:
                            # 숫자가 아닌 경우 문자열 비교
                            if str(pnli_answer).strip() == str(correct_answer).strip():
                                pnli_correct_count += 1
                                item[f"precision_nli_sc_is_correct_{weight_info}"] = True
                            else:
                                item[f"precision_nli_sc_is_correct_{weight_info}"] = False
            ...
            # 해당 가중치 설정의 precision-nli 정확도 저장
            pnli_accuracy = pnli_correct_count / total_items if total_items > 0 else 0
            pnli_accuracies[weight_info] = {
                "accuracy": pnli_accuracy,
                "correct_count": pnli_correct_count,
                "valid_count": pnli_valid_count
            }
        ...
        final_result = {
            ...
            "precision_nli_results": pnli_accuracies
        }
        
        # 결과를 저장할 파일 경로 생성 (OUTPUT_DIR 사용)
        # 여기서는 외부에서 설정된 OUTPUT_DIR을 직접 사용할 수 없으므로
        # 함수 인자로 output_dir을 전달받아 사용
        output_dir_path = os.path.join(os.path.dirname(os.path.dirname(file_path)), "precision_nli_sc_results")
        os.makedirs(output_dir_path, exist_ok=True)
        output_filename = os.path.basename(file_path).replace('.json', '_precision_nli_sc.json')
        output_file_path = os.path.join(output_dir_path, output_filename)
        
        # 처리 결과를 파일로 저장
        with open(output_file_path, 'w', encoding='utf-8') as f:
            json.dump({
                "results": precision_nli_results,
                "model_name": model_name,
                "time_info": time_info
            }, f, indent=2)
        
        print(f"처리 결과가 {output_file_path}에 저장되었습니다.")
...
def extract_model_name(filename: str) -> str:
    ...
    # 'calc_mawps_' 패턴 확인
    if filename.startswith("multiArith"):
        return filename[len("multiArith"):]
...
def main():
    ...
    # 입력 폴더 목록
    INPUT_FOLDERS = [
        "/data3/jykim/Projects/CCQA_official/Benchmarks/Multi-Arith/Result/precision_nli_separate"
    ]
    
    # 출력 디렉토리
    OUTPUT_DIR = "/data3/jykim/Projects/CCQA_official/Benchmarks/Multi-Arith/Result/precision_nli_sc_results"
...
def create_comparison_csv(all_results: List[Dict], weight_schemes: List[List[int]], output_dir: str):
    ...
    """
    # 결과 요약 CSV 생성
    csv_path = os.path.join(output_dir, "precision_nli_sc_comparison.csv")
    
    # 가중치 정보 문자열 생성
    weight_headers = []
    for weights in weight_schemes:
        weight_info = '_'.join(map(str, weights))
        weight_headers.append(f"precision_nli_sc_{weight_info}")
    ...
    with open(csv_path, 'w', newline='', encoding='utf-8') as f:
        ...
        for result in all_results:
            ...
            for weights in weight_schemes:
                weight_info = '_'.join(map(str, weights))
                if weight_info in result['precision_nli_results']:
                    accuracy = result['precision_nli_results'][weight_info]['accuracy']
                    row[f"precision_nli_sc_{weight_info}"] = f"{accuracy:.4f}"
                else:
                    row[f"precision_nli_sc_{weight_info}"] = "N/A"
    ...
    # 최상위 성능 정보 CSV 생성
    best_csv_path = os.path.join(output_dir, "best_precision_nli_models.csv")
    ...
    with open(best_csv_path, 'w', newline='', encoding='utf-8') as f:
        ...
        for weights in weight_schemes:
            weight_info = '_'.join(map(str, weights))
            writer.writerow([f"[{', '.join(map(str, weights))}]", "", "", ""])
    ...
    # 요약 테이블: 가중치별 평균 성능
    summary_csv_path = os.path.join(output_dir, "precision_nli_summary.csv")
    
    with open(summary_csv_path, 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        
        # 헤더 작성
        writer.writerow(['Weight Scheme', 'Avg Accuracy Across All Models', 'Best Overall Model', 'Best Accuracy'])
        ...
        for weights in weight_schemes:
            ...
            for result in all_results:
                if weight_info in result['precision_nli_results']:
                    all_accuracies.append(result['precision_nli_results'][weight_info]['accuracy'])
    ...
    # 유사도 방법별 요약
    similarity_csv_path = os.path.join(output_dir, "precision_nli_by_similarity.csv")
    ...
    with open(similarity_csv_path, 'w', newline='', encoding='utf-8') as f:
        ...
        for weights in weight_schemes:
            weight_info = ', '.join(map(str, weights))
            header.append(f"PNLI SC [{weight_info}]")
        ...
        for sim_method in similarity_methods:
            ...
            for weights in weight_schemes:
                ...
                for result in sim_results:
                    if weight_info in result['precision_nli_results']:
                        accuracies.append(result['precision_nli_results'][weight_info]['accuracy'])
