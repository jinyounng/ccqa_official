...
def load_calc_mawps_dataset() -> List[Dict[str, Any]]:
    """
    Load Calc-mawps dataset from Hugging Face datasets
    
    Returns:
        List of Calc-mawps questions with processed fields
    """
    # Load Calc-mawps dataset from Hugging Face
    dataset = load_dataset("MU-NLPC/Calc-mawps", "original-splits")
    ...
    for item in data:
        question_dict = {
            ...
            "original_answer": str(item.get("result", ""))
        }
...
def run_calc_mawps_benchmark(
    ...
    """
    Run benchmark on Calc-mawps dataset for a specific model and save results
    ...
    Args:
        ...
        parallel: Whether to generate responses in parallel
        
    Returns:
        Path to the saved results file
    """
    # Load dataset from Hugging Face
    questions = load_calc_mawps_dataset()
    print(f"Loaded {len(questions)} questions from Calc-mawps test dataset")
...
OUTPUT_DIR = "/data3/jykim/Projects/CCQA_official/Benchmarks/MAWPS/Result/train_result"
MAX_NEW_TOKENS = 1024
TEMPERATURE = 0.5
TOP_P = 0.9
RUN_PARALLEL = True
...
if __name__ == "__main__":
    if RUN_PARALLEL:
        ...
        output_path = run_calc_mawps_benchmark(
