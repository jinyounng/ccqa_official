...
def process_file(file_path, weights):
    ...
    time_info = data.get("time_info", {}) if isinstance(data, dict) else {}
    results = apply_original_self_consistency(results)
    results = apply_new_self_consistency(results, weights=weights)
    total = len(results)
    orig_correct = sum(is_answer_correct(item.get("original_sc_answer"), item.get("original_answer")) for item in results)
    ccqa_correct = sum(is_answer_correct(item.get("ccqa_answer"), item.get("original_answer")) for item in results)
    return {
        "model_name": model,
        "total_items": total,
...
def main():
    input_dir = "/data3/jykim/Projects/CCQA_official/Benchmarks/MAWPS/Result/ccqa_result/precision_nli_separate"
