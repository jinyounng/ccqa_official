...
def apply_original_self_consistency(results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for item in tqdm(results, desc="Applying original self-consistency", leave=False):
        ...
        # 모든 response_n 키 찾기
        responses = []
        for i in range(1,6):  # 최대 10개까지 검사
...
def apply_weighted_self_consistency(results: List[Dict[str, Any]], weights: List[int]) -> List[Dict[str, Any]]:
    ...
    for item in tqdm(results, desc=f"Applying weighted SC [{weight_info}]", leave=False):
        updated_item = item.copy()
...
def main():
    ...
    # 입력 폴더 목록
    INPUT_FOLDERS = [
        "/data3/jykim/Projects/CCQA_official/Benchmarks/GSM8K/Results/ccqa_0_result/precision_similar",
        "/data3/jykim/Projects/CCQA_official/Benchmarks/GSM8K/Results/ccqa_0_result/F1_similar",
        "/data3/jykim/Projects/CCQA_official/Benchmarks/GSM8K/Results/ccqa_0_result/cosine_similar"
    ]
    
    # 출력 디렉토리
    OUTPUT_DIR = "/data3/jykim/Projects/CCQA_official/Benchmarks/GSM8K/Results/ccqa_0_result/combined_sc_comparison"
