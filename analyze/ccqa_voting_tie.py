...
"""
 ...
 4) correct_answer, answerKey, original_answer 중 하나를 정답으로 사용
"""
...
def process_file(path: str, weights: List[int]) -> Dict[str, Any]:
    ...
    correct = 0
    for item in results:
        # 정답 필드 찾기 (correct_answer, answerKey, original_answer 순서로 확인)
        gold_answer = None
        for field in ["correct_answer", "answerKey", "original_answer"]:
            if field in item and item[field] is not None:
                gold_answer = item[field]
                break
        
        pred = vote_with_cosine(item, weights)
        item["final_answer"] = pred
        
        # 정답이 있고 예측이 맞으면 정답으로 처리
        if pred and gold_answer and numeric_equal(pred, gold_answer):
            correct += 1
    ...
    # 결과 저장 (선택 사항)
    output_path = path.replace(".json", "_processed.json")
...
WEIGHTS    = [5, 4, 4, 3, 2]           
OUTPUT_CSV = path.join(INPUT_ROOT,"summary_all_files.csv")
